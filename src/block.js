...
class Block {
    ...
    getBData() {
        ...
        return new Promise((resolve,reject)=>{
            if (self.height>0){
                let decodedBody = new Buffer.from(self.body, 'hex');
                resolve(JSON.parse(decodedBody));
            }else{
                reject(new Error('Error Getting Block data'));
            }
        });
    ...
    toString(){
        let block_data = ({
            ...
            "Block prev hash": this.previousBlockHash , 
            "Block time": this.time});
        return JSON.stringify(block_data);
    }
