...
class BlockchainController {
    ...
    constructor(app, blockchainObj) {
        ...
        this.submitStar(); 
        this.getBlockByHash(); // Not done
        this.getStarsByOwner(); 
        this.listall(); // For testing puroses
        this.validateBChain();
    }
    ...
    listall() {
        this.app.get("/listAll", async (req, res) => {
            let blocks = await this.blockchain.listAll();
            if(blocks){
                let s = "";
            ...
            if(req.body.address && req.body.message && req.body.signature && req.body.star) {
                ...
                try {
                    ...
                    if(block){
                        ...
                        return res.status(500).send(block.error);
                    }
                } catch (error) {
                    return res.status(500).send(block.error);
                }
    ...
    validateBChain() {
        this.app.get("/validatechain", async (req, res) => {
            let error_list = await this.blockchain.validateChain();
            if(error_list.length === 0){
                return res.status(404).json("All Blocks are Valid. No Errors Found");
            }else{
                return res.status(200).json(error_list);
            }
        });
    }
    ...
    getBlockByHash() {
        this.app.get("/block/hash/:hash", async (req, res) => {
            if(req.params.hash) {
                ...
                let blocks = await this.blockchain.getBlockByHash(hash);
                if(blocks){
                ...
                return res.status(404).send("Blocks Not Found! Review the Parameters!");
            }
